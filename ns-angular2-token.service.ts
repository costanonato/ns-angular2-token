...
export class NsAngular2TokenService implements CanActivate {
    ...
    private tryLoadAuthData(): void {
        ...
        let userType = this.getUserTypeByName(getString('userType'));
    ...
    private setAuthData(authData: AuthData): void {
        ...
        if (this.checkAuthData(authData)) {
            ...
            setString('accessToken', authData.accessToken);
            setString('client', authData.client);
            setString('expiry', authData.expiry);
            setString('tokenType', authData.tokenType);
            setString('uid', authData.uid);

            if (this.atCurrentUserType != null)
                setString('userType', this.atCurrentUserType.name);
